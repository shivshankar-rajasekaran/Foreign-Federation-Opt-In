...
fedreg.monitor_list = function(containerID) {
 ...
 //# Foreign Federation Optin
  //#ShivShankar Rajasekaran  24:01:2012  :: Code change for foreignfederationoptin :: Starts -->
// Foreign Federation Optin
fedreg.optin_create = function() {
	fedreg.workingOverlay();
	var dataString = $("#newoptindata").serialize();
	$.ajax({
		type: "POST",
		url: optinCreateEndpoint,
		data: dataString,
		success: function(res) {
			fedreg.optin_list();
			nimble.growl('success', res);
...
fedreg.optin_list = function(containerID) {
	$.ajax({
		type: "GET",
		cache: false,
		url: optinListEndpoint,
		success: function(res) {
			var target = $("#optin");
			target.html(res);
			applyBehaviourTo(target);
...
fedreg.optout =function(optinStatusID){
    fedreg.workingOverlay();
	var dataString = "id=" + optinStatusID;
	$.ajax({
		type: "POST",
		url: optoutEndpoint,
		data: dataString + "&_method=delete",
...
//#ShivShankar Rajasekaran  24:01:2012  :: Code change for foreignfederationoptin :: Ends -->
// Contacts
